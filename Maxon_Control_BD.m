...
%% Controller Design
    domp = -150;                            % default value – recommended
...
%     domp = -5;                              % to be tuned manually
    ...
    disp ' '
    disp (['Parameters Ki, Kp, Kd, Kii: ' num2str(contr_param)])
    Ki = contr_param (1);
    Kp = contr_param (2);
    Kd = contr_param (3);
    Kii= contr_param (4);
    save Maxon_Control.mat domp Ki Kp Kd Kii A3 B3 C3 D3
% Continue with block diagram PID_RE35.slx
% or with script Maxon_feedback.m
